...
describe('hovercraft', function()
  ...
  describe('scrollbar calculation', function()
    it('returns should_show = false when content fits inside window', function()
      local res = UI._calculate_scrollbar(5, 10, 1)
      is_false(res.should_show)
      eq(0, res.bar_size)
      eq(0, res.bar_pos)
      eq({}, res.lines)
    end)

    it('returns should_show = false when total equals window height', function()
      local res = UI._calculate_scrollbar(10, 10, 1)
      is_false(res.should_show)
      eq(0, res.bar_size)
    end)
